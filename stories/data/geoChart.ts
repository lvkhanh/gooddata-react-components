...
const getMeasureHeaderItem = (name: string, uri: string): Execution.IMeasureHeaderItem => ({
    measureHeaderItem: {
        name,
        format: "#,##0",
        localIdentifier: `m_${name}`,
        uri,
...
export function getExecutionResponse(
    ...
    if (isWithSize) {
        measureHeaderItems.push(getMeasureHeaderItem("Population", "/gdc/md/projectId/obj/4"));
    }

    if (isWithColor) {
        measureHeaderItems.push(getMeasureHeaderItem("Area", "/gdc/md/projectId/obj/5"));
    }
