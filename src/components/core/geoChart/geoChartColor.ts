...
export function getColorPaletteMapping(segmentItems: string[] = []): IObjectMapping {
    ...
    const colorPalette = segmentItems.reduce(
        (result: string[][], _item: string, index: number): string[][] => {
            const colorPalette =
                index < DEFAULT_COLORS.length
                    ? getColorPalette(DEFAULT_COLORS[index])
                    : result[index - DEFAULT_COLORS.length]; // colors loop every DEFAULT_COLORS.length
            return result.concat([colorPalette]);
        },
        [],
    );
...
export function getPushpinColors(
    ...
    const colorsWithoutNull = compact(colors);
    ...
    return colors.map(
        (color: number, index: number): IPushpinColor => {
            const value = !color || isNaN(color) ? min : color;
